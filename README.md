...
**Audio.** Every one of the 754 cards has a clip in the public `audio` bucket,
put there once by `scripts/generate-audio.mjs` — never at runtime. The 724 word
cards are Azure neural TTS, split between a female and a male Serbian voice; the
30 letter cards are **Mark's wife's own recordings**, which no engine may
overwrite (see the warning under *Seed it*). A card with a clip gets a speaker
button; a card without one gets no button, rather than a button that does
nothing.
...
For the audio batch only: an Azure Speech key, or Piper and ffmpeg for the
offline fallback — see [`docs/plans/audio-batch.md`](docs/plans/audio-batch.md).
...
Audio is optional and separate, because it needs a TTS engine or a key:

```sh
npm run db:audio          # speak the 724 word cards (Azure), upload, fill in cards.audio_path
```

It needs `AZURE_SPEECH_KEY` and `AZURE_SPEECH_REGION` (`.env.example` says
where); `npm run db:audio -- --engine=piper` is the offline fallback and needs
no key. **Either way it leaves the 30 letter cards alone** — those clips are
human, and the script refuses to speak that deck without an explicit `--letters`
flag it also warns you about.

Skip it and the app works exactly as it does now, minus the speaker buttons.
[`docs/plans/audio-batch.md`](docs/plans/audio-batch.md) has the credentials, the
one-time Piper install, and the rerun rules.
...
> Mark's wife's voice, recorded once, and no engine can regenerate them.
> `db:audio` therefore skips the letter deck entirely unless it is given
> `--letters`, which it warns loudly about — the only way to lose the
> recordings is to insist. The clips live in `recordings/letter-clips/`
> (git-ignored, this machine only) and in the hosted `audio` bucket at
> `cards/<id>-w1.mp3`; after a local reset, re-upload them from there
> rather than pointing TTS at the letter deck.
